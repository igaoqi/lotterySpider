...
namespace LotterySpider.Core.Jobs.Impls.JingWaiCai
{
    ...
    public sealed class TwbgJob : IJobProvider<List<Jwc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Twbg_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jwc_Twbg LIMIT 1";
        ...
        {
            Console.WriteLine("Twbg" + DateTime.Now);
        ...
        public void Insert(List<Jwc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jwc_Twbg(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble,DragonTiger) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble,@DragonTiger)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJWC>>>(string.Format(Url, date, LotteryCode.JingWaiCai.Twbg), 10);
