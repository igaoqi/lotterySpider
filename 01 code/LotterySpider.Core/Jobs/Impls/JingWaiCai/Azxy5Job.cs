...
namespace LotterySpider.Core.Jobs.Impls.JingWaiCai
{
    ...
    public sealed class Azxy5Job : IJobProvider<List<Jwc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Azxy5_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jwc_Azxy5 LIMIT 1";
        ...
        {
            Console.WriteLine("Azxy5" + DateTime.Now);
        ...
        public void Insert(List<Jwc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jwc_Azxy5(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble,DragonTiger) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble,@DragonTiger)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJWC>>>(string.Format(Url, date, LotteryCode.JingWaiCai.Azxy5), 10);
