...
namespace LotterySpider.Core.Jobs.Impls.Shiyix5
{
    ...
    public sealed class Shiyi5LnJob : IJobProvider<List<Shiyi5>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Shiyi5_Ln_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM Shiyi5_Ln LIMIT 1";
        ...
        public void Insert(List<Shiyi5> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO Shiyi5_Ln(Issue,preDrawCode,DrawTime,DrawDate,SingleOrDouble,DragonTiger,SumNum,BehindThree,BetweenThree,LastThree) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SingleOrDouble,@DragonTiger,@SumNum,@BehindThree,@BetweenThree,@LastThree)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubShiyi5>>>(string.Format(Url, date, LotteryCode.Shiyi5.Ln));
