...
namespace LotterySpider.Core.Jobs.Impls.K3
{
    ...
    public sealed class K3HbJob : IJobProvider<List<K3Model>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_K3_Hb_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM K3_Hb LIMIT 1";
        ...
        public void Insert(List<K3Model> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO K3_Hb (Issue,preDrawCode,DrawTime,DrawDate,SumNum,sumBigSmall,SingleOrDouble,firstSeafood,secondSeafood,thirdSeafood)
                                        ...
                                        WHERE NOT EXISTS(SELECT 1 FROM K3_Hb WHERE Issue =@Issue) ";
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubK3>>>(string.Format(Url, date, LotteryCode.K3.Hb));
