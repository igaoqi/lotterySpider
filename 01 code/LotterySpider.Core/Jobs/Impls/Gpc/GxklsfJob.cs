...
namespace LotterySpider.Core.Jobs.Impls.Gpc
{
    ...
    public sealed class GxklsfJob : IJobProvider<List<GpcModel>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_gpc_Gxklsf_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM gpc_Gxklsf LIMIT 1";
        ...
        public void Insert(List<GpcModel> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO gpc_Gxklsf (Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble,DragonTiger)
                                        SELECT @Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble,@DragonTiger FROM DUAL
                                        WHERE NOT EXISTS(SELECT 1 FROM gpc_Gxklsf WHERE Issue =@Issue) ";
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubGpc>>>(string.Format(Url, date, LotteryCode.Gpc.Gxklsf));
