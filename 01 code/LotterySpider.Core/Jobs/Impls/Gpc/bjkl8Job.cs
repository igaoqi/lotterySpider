...
namespace LotterySpider.Core.Jobs.Impls.Gpc
{
    ...
    public sealed class Bjkl8Job : IJobProvider<List<GpcModel>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_gpc_Bjkl8_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM gpc_Bjkl8 LIMIT 1";
        ...
        public void Insert(List<GpcModel> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO gpc_Bjkl8 (Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble)
                                        SELECT @Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble FROM DUAL
                                        WHERE NOT EXISTS(SELECT 1 FROM gpc_Bjkl8 WHERE Issue =@Issue) ";
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubGpc>>>(string.Format(Url, date, LotteryCode.Gpc.Bjkl8));
