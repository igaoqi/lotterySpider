...
namespace LotterySpider.Core.Jobs.Impls.Hot
{
    ...
    public sealed class PCLucky28Job : IJobProvider<List<Ssc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_PCLucky28_{0}";
            }
        }
        ...
        public string Url
        {
            get
            {
                return ConfigHelper.GetString("168LotteryAPI") + "LuckTwenty/getPcLucky28List.do?date={0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM hot_PCLucky28 LIMIT 1";
        ...
        public void Insert(List<Ssc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO hot_PCLucky28(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubRmc>>>(string.Format(Url, date));
