...
namespace LotterySpider.Core.Jobs.Impls.Hot
{
    ...
    public sealed class SGftJob : IJobProvider<List<Rmc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_SGft_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM hot_SGft LIMIT 1";
        ...
        public void Insert(List<Rmc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO hot_SGft(Issue,preDrawCode,DrawTime,DrawDate,SumFS,SingleOrDouble,DragonTiger) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumFS,@SingleOrDouble,@DragonTiger)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubRmc>>>(string.Format(Url, date, LotteryCode.Hot.Sgft));
