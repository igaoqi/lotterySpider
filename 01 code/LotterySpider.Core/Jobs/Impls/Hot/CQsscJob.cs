...
namespace LotterySpider.Core.Jobs.Impls.Hot
{
    ...
    public sealed class CQsscJob : IJobProvider<List<Ssc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_CQssc_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM hot_CQssc LIMIT 1";
        ...
        public void Insert(List<Ssc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO hot_CQssc(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble,DragonTiger) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble,@DragonTiger)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubRmc>>>(string.Format(Url, date, LotteryCode.Hot.CQssc));
