...
namespace LotterySpider.Core.Jobs.Impls.QuanGuoCai
{
    ...
    public sealed class QlcJob : IJobProvider<List<Qgc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Qlc_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM qgc_Qlc LIMIT 1";
        ...
        {
            Console.WriteLine("Qlc" + DateTime.Now);
        ...
        public void Insert(List<Qgc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO qgc_Qlc(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubQGC>>>(string.Format(Url, date, LotteryCode.QuanGuoCai.Qlc));
