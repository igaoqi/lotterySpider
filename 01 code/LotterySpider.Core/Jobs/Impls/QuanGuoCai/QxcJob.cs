...
namespace LotterySpider.Core.Jobs.Impls.QuanGuoCai
{
    ...
    public sealed class QxcJob : IJobProvider<List<Qgc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Qxc_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM qgc_Qxc LIMIT 1";
        ...
        {
            Console.WriteLine("Qxc" + DateTime.Now);
        ...
        public void Insert(List<Qgc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO qgc_Qxc(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubQGC>>>(string.Format(Url, date, LotteryCode.QuanGuoCai.Qxc));
