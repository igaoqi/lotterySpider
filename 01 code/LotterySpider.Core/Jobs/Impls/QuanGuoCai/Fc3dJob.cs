...
namespace LotterySpider.Core.Jobs.Impls.QuanGuoCai
{
    ...
    public sealed class Fc3dJob : IJobProvider<List<Qgc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Fc3d_{0}";
            }
        }
        ...
        public string Url
        {
            get
            {
                return ConfigHelper.GetString("168LotteryAPI") + "QuanGuoCai/getLotteryInfoList.do?date={0}&lotCode={1}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM qgc_Fc3d LIMIT 1";
        ...
        {
            Console.WriteLine("Fc3d" + DateTime.Now);
        ...
        public void Insert(List<Qgc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO qgc_Fc3d(Issue,preDrawCode,DrawTime,DrawDate,SumNum,SingleOrDouble) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SumNum,@SingleOrDouble)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubQGC>>>(string.Format(Url, date, LotteryCode.QuanGuoCai.Fc3q));
