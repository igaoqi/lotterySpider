...
namespace LotterySpider.Core.Jobs.Impls.JiSuCai
{
    ...
    public sealed class Js11x5Job : IJobProvider<List<Jsc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Js11x5_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jsc_Js11x5 LIMIT 1";
        ...
        {
            Console.WriteLine("Js11x5" + DateTime.Now);
        ...
        public void Insert(List<Jsc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jsc_Js11x5(Issue,preDrawCode,DrawTime,DrawDate,SingleOrDouble,DragonTiger,SumNum) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SingleOrDouble,@DragonTiger,@SumNum)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJSC>>>(string.Format(Url, date, LotteryCode.JiSuCai.Js11x5));
