...
namespace LotterySpider.Core.Jobs.Impls.JiSuCai
{
    ...
    public sealed class Jskl8Job : IJobProvider<List<Jsc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Jskl8_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jsc_Jskl8 LIMIT 1";
        ...
        {
            Console.WriteLine("Jskl8" + DateTime.Now);
        ...
        public void Insert(List<Jsc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jsc_Jskl8(Issue,preDrawCode,DrawTime,DrawDate,SingleOrDouble,DragonTiger,SumNum) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SingleOrDouble,@DragonTiger,@SumNum)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJSC>>>(string.Format(Url, date, LotteryCode.JiSuCai.Jskl8));
