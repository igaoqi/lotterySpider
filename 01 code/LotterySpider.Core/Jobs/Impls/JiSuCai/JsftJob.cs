...
namespace LotterySpider.Core.Jobs.Impls.JiSuCai
{
    ...
    public sealed class JsftJob : IJobProvider<List<Jsc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Jsft_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jsc_Jsft LIMIT 1";
        ...
        {
            Console.WriteLine("Jsft" + DateTime.Now);
        ...
        public void Insert(List<Jsc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jsc_Jsft(Issue,preDrawCode,DrawTime,DrawDate,SingleOrDouble,DragonTiger,SumNum) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SingleOrDouble,@DragonTiger,@SumFS)
                               ON DUPLICATE KEY UPDATE preDrawCode=@preDrawCode,DrawTime=@DrawTime,DrawDate=@DrawDate,SingleOrDouble=@SingleOrDouble,DragonTiger=@DragonTiger,SumNum=@SumFS";
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJSC>>>(string.Format(Url, date, LotteryCode.JiSuCai.Jsft));
