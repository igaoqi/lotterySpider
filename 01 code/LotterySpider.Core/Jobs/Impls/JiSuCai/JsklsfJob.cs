...
namespace LotterySpider.Core.Jobs.Impls.JiSuCai
{
    ...
    public sealed class JsklsfJob : IJobProvider<List<Jsc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Jsklsf_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jsc_Jsklsf LIMIT 1";
        ...
        {
            Console.WriteLine("Jsklsf" + DateTime.Now);
        ...
        public void Insert(List<Jsc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jsc_Jsklsf(Issue,preDrawCode,DrawTime,DrawDate,SingleOrDouble,DragonTiger,SumNum) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SingleOrDouble,@DragonTiger,@SumNum)
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJSC>>>(string.Format(Url, date, LotteryCode.JiSuCai.Jsklsf));
