...
namespace LotterySpider.Core.Jobs.Impls.JiSuCai
{
    ...
    public sealed class Jsk3Job : IJobProvider<List<Jsc>>
    {
        public string CacheKey
        {
            get
            {
                return "Cachekey_Jsk3_{0}";
            }
        }
        ...
        public DateTime JobDate
        {
            get
            {
                string sql = "SELECT MAX(DRAWTIME) DATE FROM jsc_Jsk3 LIMIT 1";
        ...
        {
            Console.WriteLine("Jsk3" + DateTime.Now);
        ...
        public void Insert(List<Jsc> models)
        {
            ...
            using (var conn = new MySqlConnection(DbFactory.GetConnString()))
            {
                string sql = @"INSERT INTO jsc_Jsk3(Issue,preDrawCode,DrawTime,DrawDate,SingleOrDouble,SumNum) VALUES (@Issue,@preDrawCode,@DrawTime,@DrawDate,@SingleOrDouble,@SumNum)
                               ON DUPLICATE KEY UPDATE preDrawCode=@preDrawCode,DrawTime=@DrawTime,DrawDate=@DrawDate,SingleOrDouble=@SingleOrDouble,SumNum=@SumNum";
        ...
        {
            var data = RequestHelper.Get<Json168Root<List<Json168SubJSC>>>(string.Format(Url, date, LotteryCode.JiSuCai.Jsk3));
